 ...
 FROM (SELECT cs.cs_item_sk as cs_item_sk,
             ...
             WHERE d.d_date between '2000-02-12' and date_add('2000-02-12',90)) cs1
 ...
 JOIN (SELECT cs2.cs_item_sk as cs_item_sk,
           ...
           FROM (SELECT cs.cs_item_sk as cs_item_sk,
                        ...
                        FROM catalog_sales cs
                        JOIN date_dim d ON (d.d_date_sk = cs.cs_sold_date_sk)
                        WHERE d.d_date between '2000-02-12' and date_add('2000-02-12',90)) cs2
                        GROUP BY cs2.cs_item_sk) tmp1
 ON (i.i_item_sk = tmp1.cs_item_sk)
 WHERE i.i_manufact_id = 787 and
